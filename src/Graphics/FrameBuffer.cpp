...
FrameBuffer::FrameBuffer(std::string&& name, uint16_t width, uint16_t height, TextureFormat colorFormat,
    ...
    , _colorAttachment(BGFX_INVALID_HANDLE)
    , _depthStencilAttachment(BGFX_INVALID_HANDLE)
{
	if (depthStencilFormat)
	{
		...
		_colorAttachment = fromBgfx(bgfx::getTexture(toBgfx(_handle), 0));
		_depthStencilAttachment = fromBgfx(bgfx::getTexture(toBgfx(_handle), 1));
	}
	else
	{
