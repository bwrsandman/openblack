...
void OldCameraModel::HandleActions(std::chrono::microseconds dt)
{
	...
	{
		auto mousePosition = actionSystem.GetMousePosition();
		auto dist = glm::distance(glm::vec2(mousePosition), glm::vec2(_mouseFirstClick));
	...
	if (actionSystem.GetMouseDelta() != glm::zero<glm::ivec2>())
	{
		if (actionSystem.Get(input::BindableActionMap::ROTATE_AROUND_MOUSE_ON))
		{
			...
			else // Holding down the middle mouse button without shift enables hand orbit camera rotation.
			{
				...
				const float yaw = delta.x * (glm::two_pi<float>() / size.x);
				float pitch = delta.y * (glm::pi<float>() / size.y);
