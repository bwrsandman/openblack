...
protected:
	void SetUp() override
	{
		...
		game_ = std::make_unique<Game>(std::move(args));
...
TEST_F(TestFixed, celtic_town_centre_mesh_bounding_circle)
{
...
TEST_F(TestFixed, celtic_town_centre_bounding_circle)
{
	...
	auto& fixed = game_->GetEntityRegistry().Get<const Fixed>(entity);
	ASSERT_EQ(fixed.boundingCenter, glm::vec2(2188.182129f, 2317.253906f));
	ASSERT_EQ(fixed.boundingRadius, 12.9809408f);
}

TEST_F(TestFixed, celtic_abode_mesh_bounding_circle)
{
	Transform transform {{}, glm::identity<glm::mat3>(), {1.0f, 1.0f, 1.0f}};
...
TEST_F(TestFixed, celtic_abode_bounding_circle)
{
	TownArchetype::Create(0, glm::vec3(2185.72f, 0.0f, 2315.78f), PlayerNames::PLAYER_ONE, Tribe::CELTIC);
...
TEST_F(TestFixed, celtic_abode_bounding_circle_scaled)
{
